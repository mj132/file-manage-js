...
function success (data = {}, message = '') { return { code: 200, data, message } }
...
function fileHasSync (dirPath) {
  ...
  if (res.code === 200 && res.data.type === 'file') return success()
...
function dirHasSync (dirPath) {
  dirPath = path.join(dirPath)
  const res = existsSync(dirPath)
  if (res.code === 200 && res.data.type === 'dir') return success()
...
function readDirSync (dirPath) {
  ...
  return success(fs.readdirSync(dirPath))
}
...
function deletePath (delPath) {
  return new Promise(async function (resolve) {
    ...
    if (res.code !== 200) return resolve(success())
...
function mkDirSync (dirPath) {
  dirPath = path.join(dirPath)
  if (dirHasSync(dirPath).code === 200) return success({}, 'The folder already exists')
...
function mkDir (dirPath) {
  return new Promise(async function (resolve) {
    dirPath = path.join(dirPath)
    if ((await dirHas(dirPath)).code === 200) return resolve(success({}, 'The folder already exists'))
...
function readFileSync (filePath) {
  ...
  return success(fs.readFileSync(filePath, 'utf-8'))
}
